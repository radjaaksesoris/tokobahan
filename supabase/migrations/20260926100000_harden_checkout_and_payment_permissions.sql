-- Harden the latest checkout implementation without rewriting migration history.
-- This restores role checks, FIFO costing, server-side totals, and retry idempotency.
...
BEGIN
  ...
  -- This also covers a committed transaction whose HTTP response was lost.
  SELECT ci.sale_id INTO sale_id
  FROM public.checkout_idempotency ci
  WHERE ci.invoice_no = p_invoice_no;
  IF sale_id IS NOT NULL THEN RETURN sale_id; END IF;

  SELECT s.id INTO sale_id FROM public.sales s WHERE s.invoice_no = p_invoice_no;
  IF sale_id IS NOT NULL THEN
    INSERT INTO public.checkout_idempotency(invoice_no, sale_id)
    VALUES (p_invoice_no, sale_id)
  ...
  -- Keep the same-day credit merge, but lock the selected sale and use only
  ...
  IF sale_id IS NULL THEN
    ...
    VALUES (p_invoice_no, calculated_total_amount, calculated_total_cost,
      calculated_total_amount - calculated_total_cost, p_payment_method, p_cashier_id, customer_id, paid)
  ...
  FOR item IN SELECT row_to_json(f)::JSONB FROM fifo_checkout_items f LOOP
    ...
    IF existing_item.id IS NULL THEN
      ...
      VALUES (sale_id, (item->>'product_id')::UUID, item->>'product_name', item->>'unit',
...
BEGIN
  IF NOT public.current_user_has_role(ARRAY['admin', 'cashier']) THEN
    RAISE EXCEPTION 'Hanya admin atau cashier yang dapat mengubah metode pembayaran';
  END IF;
  IF auth.uid() IS NULL OR p_payment_method NOT IN ('cash', 'credit') THEN
    RAISE EXCEPTION 'Permintaan perubahan pembayaran tidak valid';
  END IF;

  SELECT * INTO sale_record FROM public.sales WHERE id = p_sale_id FOR UPDATE;
  IF sale_record.id IS NULL THEN RAISE EXCEPTION 'Transaksi tidak ditemukan'; END IF;
  IF sale_record.payment_method = p_payment_method THEN RETURN; END IF;
  ...
  IF p_payment_method = 'credit' THEN
    ...
    UPDATE public.sales SET payment_method = 'credit', customer_id = customer_record, amount_paid = 0 WHERE id = p_sale_id;
    RETURN;
  END IF;

  SELECT COUNT(*) INTO payment_count FROM public.customer_debt_payments WHERE sale_id = p_sale_id;
  IF payment_count > 0 THEN
    RAISE EXCEPTION 'Transaksi kredit yang sudah memiliki cicilan tidak dapat diubah menjadi tunai';
  END IF;
  UPDATE public.sales SET payment_method = 'cash', customer_id = NULL, amount_paid = total_amount WHERE id = p_sale_id;
END;
$$;
